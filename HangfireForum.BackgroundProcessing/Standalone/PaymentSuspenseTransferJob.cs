...
{
    public class PaymentSuspenseTransferJob : BaseJobAsync<SuspenseTransferJobArgs>
    {
        private readonly ISuspenseTransferService _suspenseTransferService;

        public PaymentSuspenseTransferJob(ISuspenseTransferService suspenseTransferService)
        {
        ...
        {
            var result = await this._suspenseTransferService.TransferToSuspense(jobParams.PaymentId);
